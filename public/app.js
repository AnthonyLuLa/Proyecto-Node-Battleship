document.addEventListener('DOMComponentLOaded',() =>{
    ...
    const turnDisplay = document.querySelector('#quien va');
...
// single player
    function startSinglePlayer() {
        ...
        startButton.addEventListener('click', () => {
            setupButtons.style.diplay = 'none'
            playGameSingle()
        })
    }
// drawing cpu ships randomly
    function general(ship){
        let randomDirection = Math.floor(math.random() * ship.
        direction.length);
        let current = ship.directions[randomDirections];
        if (randomDirections === 0) directions = 1;
        if (randomDirections === 0) directions = 10;

        const istTaken = current.some(index => cpuSquares[index].
            classList.contain('taken'));

        const isAtRightEdge = current.some(index => (index) % width
        === width - 1)
        const isAtLeftEdge = current.some(index => (index) % width
        === width - 0)
    }
// multiplayer
    function startMultiPlayer() {
        const socket = io();
// player number in the server
...
// check the player status
        socket.on('check-players', players => {
            player.forEach((p, i) => {
                if (p.connected) playerConnected(i);
                if (p.ready) {
                    playerReady(i);
...
// click for ready button
        startButton.addEventListener('click', () => {
            if(allShipsPlaced) playGAmeMulti(socket)
            else infoDisplay.innerHTML = "por favor coloca TODOS los barcos";
                
        });
// listeners for shooting
        cpuSquares.forEach(square => {
            square.addEventListener('click', () => {
                if (currentPLayer === 'user' && ready && enemyReady){
                    shootsFire = square.dataset.id;
                    socket.emit('fire',shootsFired)    
                }
            });
        });
// got shotted
